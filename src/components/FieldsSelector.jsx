...
export default function FieldsSelector({ selected = [], setSelected = () => {} }) {
  ...
  return (
    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-lg backdrop-blur-sm p-6 sticky top-6">
      ...
      <div className="mb-4">
        <button onClick={applyChanges} onKeyDown={handleKeyDown} className={`w-full px-4 py-3 text-sm font-medium rounded-lg transition-all duration-300 shadow-lg ${tempSelected.length === 0 ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:from-purple-500 hover:to-pink-500'}`} disabled={tempSelected.length === 0}>
          View results
        </button>
      </div>
